# 3D globe, ships and battle scenes — 0.38.0

Review date: 24 September 2026. The production map is a WebGL 2 globe with one continuous perspective camera, spherical terrain and geographic graticules. Close-up ships use actual cuboid meshes from editable model files. Camera and formation state remain separate from campaign positions and calculations.

- Standard regression: **417/417 checks passed**. Catalog validation covers 86 live documents and all 14 starting states. This graphics update does not change campaign mechanics; the earlier long-campaign evidence below is retained rather than presented as a new endurance run.
- Both map geometries pass land-triangle radius and winding checks, including date-line crossings, polar closure, polygon holes and raycasts. No land triangle crosses below the ocean. Camera tests cover all hemispheres, strategic and physical ship scales, repeated longitude wrapping, orbit, safe near planes, and pointer-anchored dragging at different tilts and offset viewports.
- Asset checks cover **181 voxel models**, 166 class types, 281 campaign mappings, 14 generic type fallbacks and all 244 opening legacy groups. Every surviving hull in the player's active convoys is represented; tests include a 1,201-hull convoy without an arbitrary display cap. Hidden enemy fleets are excluded. Mesh tests verify all cuboids, normals, winding, dimensions, materials and shared geometry.
- Three.js r180 is bundled with its MIT notice. All three vendor files match the pinned upstream bytes and SHA-256 values, checked against the official sources. The local server permits only the two exact vendor JavaScript module paths; the existing self-only content policy remains in force. Recognition and music asset audits also pass.
- The compact interface and opening demonstrations pass real Edge checks at 1920, 1366, 1100, 900 and 768 pixels, including peace/war resource hovers, ministry close controls, pending alerts, all navy selections, reduced motion, demo hull selection, modal suspension, and save isolation. The run records **zero page errors**.
- Globe browser checks verify actual WebGL 2 rendering, repeated date-line wrapping, same-size title/continue remount, real triangle selection and rejection of empty air inside model bounds. Wide and narrow windows reach **65,536×**, including a separate 200% display-scale context with a 3840×2000 backing buffer. Actual canvas hover/click inspects the correct warship group and exact merchant hull index. Camera-only input leaves campaign time, RNG and resources unchanged. All runtime requests remain local.
- Battle browser checks verify pause on opening, exactly 15 minutes of whole-campaign advancement from live Next, read-only previous/next navigation, and recorded frames after save/reload. Visual review corrected an overlay positioning error and increased the opening fleet framing. Final labels stay within the canvas. Focused regressions cover perspective orbit, pan, anchored zoom, actual hull triangle clicks and zero-hull air actions.
- The final native executable passes **USA 1936 and Britain 1922** with no Node on PATH, all 181 models, all 33 music tracks, recognition artwork, real WebGL rendering, extreme zoom, exact ship/merchant selection, domestic port hovers, diplomacy, funding, save/close/reopen and temporary cleanup. A five-round surface battle passes live stepping, mid-battle save/reopen, recorded navigation and completion. Native runs record **zero JavaScript errors and zero external runtime requests**. Native close-up and battle screenshots were visually reviewed. Tests were updated to rotate to the appropriate hemisphere, identify the frontmost overlapping fleet, and distinguish domestic port information from foreign intelligence.
- Packaging verifies **788 source files / 861 payload files**. Executable: `WNT1922-0.38.0-portable-win-x64.exe`, 476,905,435 bytes. SHA-256: `f67e9164f27baf3e9e699d2488accc664ca633612870fa842941b55bfdc4146b`. Source fingerprint: `779addacf6ff2e2864a43748327cc7a9f76485d3b69f3b41e8017e10fa46295e`.

Close-up harbour formations are arranged seaward for inspection and disclosed as illustrative. They do not replace strategic positions. Merchant meshes represent hull counts; voyage cargo and condition remain convoy-level information. Battle formations and impacts remain illustrative displays of recorded results. The renderer requires WebGL 2 graphics acceleration.

Repeatable checks: `tools/check-globe-ui.mjs`, `tools/check-chrome-ui.mjs`, `tools/check-portable.mjs`, `tools/audit-assets.mjs` and the regression suite. Local evidence is under ignored `test-output/globe-ui/`, `test-output/chrome-ui/`, `test-output/portable/` and `test-output/release38-*`.
