...
function observed(s) {
  ...
  const f = s.nations.USA.fleets[0];
...
test("Contact alerts refresh known information, expire at staleness, and do not reveal live positions", () => {
  const s = start(),
    ...
    entry = alertItems(s).find((a) => a.id === id);
  assert.match(entry.body, /132.0°E/);
  assert.match(entry.body, /approximately/);
  s.nations.USA.fleets[0].route = [[15, 60]];
  assert.match(alertItems(s).find((a) => a.id === id).body, /132.0°E/);
  setCampaignMinutes(s, campaignMinutes(s) + 48 * 60);
  ...
  setCampaignMinutes(s, campaignMinutes(s) + 120 * 60);
  assert.equal(visibleContacts(s).length, 0);
});
test("Contact dismissal survives tracking and saves; reacquisition after a stale gap alerts again", () => {
  const s = start(),
    ...
    id = "contact-" + contact.id;
  sim.dismissNotice(s, c, id);
  assert.equal(contactAlerts(s).length, 0);
  validateSave(s, c);
  setCampaignMinutes(s, campaignMinutes(s) + 15);
  recordContact(s, "JPN", "USA", f, [133, 6], fleetStats(s, c, "USA", f));
  assert.equal(contactAlerts(s).length, 0);
  setCampaignMinutes(s, campaignMinutes(s) + 49 * 60);
  recordContact(s, "JPN", "USA", f, [136, 8], fleetStats(s, c, "USA", f));
  assert.equal(contactAlerts(s).length, 1);
  sim.clearOptionalAlerts(s, c);
  assert.equal(contactAlerts(s).length, 0);
  assert.ok(s.decisions.every((d) => d.critical));
  validateSave(s, c);
});
test("Player fleet missions are independent of hidden national priorities; AI missions follow peace and war", () => {
  const s = start(),
    n = s.nations.JPN,
    f = n.fleets.find((f) => f.role === "battle");
  assert.ok(f);
  f.manual = false;
  f.mission = "presence";
  n.priority = "decisive";
  applyStandingOrders(s, c, "JPN");
  assert.equal(f.mission, "presence");
  const ai = s.nations.USA,
    other = ai.fleets.find((f) => f.role === "battle");
  other.manual = false;
  other.mission = "presence";
  ai.priority = "decisive";
  applyStandingOrders(s, c, "USA");
  assert.equal(other.mission, "presence");
  s.relations["JPN-USA"].war = true;
  other.salt = 1;
  applyStandingOrders(s, c, "USA");
  assert.equal(other.mission, "decisive");
});
test("Suggested fits follow assigned missions, without a hidden national directive", () => {
  const s = start(),
    n = s.nations.JPN;
  for (const f of n.fleets) f.mission = "guard";
  const fit = automaticDraft(s, c, "DD");
  assert.ok(fit.torpedoes <= 6);
  n.priority = "decisive";
  assert.deepEqual(automaticDraft(s, c, "DD"), fit);
});
