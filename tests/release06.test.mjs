...
import { NODES, PORTS } from "../mechanics/world.mjs";
import {
  portSummary,
  damagePort,
  repairPorts,
...
import {
  minutePortOperations,
  anchoredShips,
} from "../mechanics/port-operations.mjs";
import {
  orderAircraft,
  aircraftModels,
  setProductionModel,
...
const bridge = () => {
  ...
  w.on("message", (data) => out.onmessage?.({ data }));
  ...
  for (const campaign of Object.keys(b.campaigns))
    for (const id of Object.keys(b.nations)) {
      ...
      for (const [key, p] of Object.entries(PROGRAMS)) {
        ...
        assert.equal(price.days, 180);
        assert.ok(price.gold >= p.gold);
        n.tech[p.level] = 9;
        assert.match(sim.projectBlock(s, key), /Level 9/);
        n.tech[p.level] = 1;
      }
      validateSave(s, b);
    }
...
test("catalog dates block ship and aircraft orders; research retains its availability gates", () => {
  const s = start(),
    c = contentFor(b, s),
    n = s.nations.JPN;
  n.gold = n.industry = n.influence = 1e8;
...
test("tiny rounded time intervals return promptly and repeated resume/pause uses a real worker", async (t) => {
  ...
  assert.equal(sim.advanceMinutes(s, b, 0.0000005), 0);
  assert.equal(campaignMinutes(s), before);
  for (let i = 0; i < 12; i++) {
    await client.dispatch({
      type: "speed",
      args: { value: [0.25, 0.5, 1, 5, 10][i % 5] },
    });
  ...
  assert.ok(campaignMinutes(after) > before);
...
test("worker display failures pause safely, reject pending requests and permit a fresh resume", async (t) => {
  ...
  const client = new SimulationClient({
    createWorker: bridge,
    onState: () => {
      if (fail) throw Error("Injected render failure");
    },
    onError: (m) => (message = m),
  });
  t.after(() => {
  ...
  await assert.rejects(client.start(b, start()), /render failure/);
  assert.equal(client.worker, null);
  assert.equal(client.current.paused, true);
  assert.match(message, /preserved/);
  fail = false;
  await client.start(b, client.current);
  await client.dispatch({ type: "pause", args: { value: false } });
  await client.stop();
});
test("sailor shortages leave whole ships in port; casualties at sea reduce weapon effectiveness and survivors remain conserved", () => {
  const s = start("GBR"),
    ...
    groups = n.groups.filter((g) => g.fleetId === f.id),
    g = groups[0];
  orderFleet(s, c, f.id, "presence");
  assert.equal(g.atSea, true);
  const positions = fleetPosition(s, f),
    power = sim.fleetPower(s, c, "GBR", null, f.id).surface,
    people = n.crew;
  const result = sim.damageFleet(
    ...
    g.region,
    0.45,
    { surface: 500, air: 0, asw: 0, sub: 0 },
    "gunfire",
    f.id,
    0.5,
  );
  assert.equal(people - n.crew, result.sailorsLost + result.sailorsRescued);
  assert.ok(
    groups.some(
      (g) => g.count && crewEffectiveness(g, c.classes[g.classId]) < 1,
    ),
  );
  assert.ok(sim.fleetPower(s, c, "GBR", null, f.id).surface < power);
  assert.deepEqual(fleetPosition(s, f), positions);
  const another = start("USA"),
    u = another.nations.USA;
  u.crew = 1000;
  staffSailors(another, c, "USA");
  assert.ok(sailorSummary(another, c, "USA").waiting > 0);
  for (const g of u.groups.filter(
    (g) => g.status === "active" && !g.atSea && g.service === "warship",
  ))
    assert.ok(g.sailors === 0 || fullyStaffed(g, c.classes[g.classId]));
  const force = u.fleets.find((f) =>
    u.groups.some((g) => g.fleetId === f.id && !g.sailors),
  );
  orderFleet(another, c, force.id, "presence");
  assert.ok(
    !u.groups.some(
      (g) => g.fleetId === force.id && !fullyStaffed(g, c.classes[g.classId]),
    ),
  );
});
test("submarine torpedo handling also falls with sailor casualties at sea", () => {
  const s = start("DEU"),
    c = contentFor(b, s),
    n = s.nations.DEU,
    f = n.fleets.find((f) => f.role === "submarine");
  assert.ok(f);
  Object.assign(s.relations["DEU-GBR"], { war: true, warSince: s.day });
  orderFleet(s, c, f.id, "raid");
  const before = sim.fleetPower(s, c, "DEU", null, f.id).sub;
  for (const g of n.groups.filter((g) => g.fleetId === f.id)) {
    const lost = Math.floor(g.sailors / 2);
    g.sailors -= lost;
    n.crew -= lost;
  }
  const after = sim.fleetPower(s, c, "DEU", null, f.id).sub;
  assert.ok(after < before * 0.6 && after > before * 0.4);
  validateSave(s, b);
});
test("port attacks damage facilities and anchored ships, report casualties, and repair only after safety and payment", () => {
  ...
  const enemy = s.nations.USA.fleets.find((f) => f.role === "carrier");
  enemy.port = "hawaii";
  enemy.phase = "port";
  enemy.route = [NODES.hawaii];
  enemy.arriveAt = campaignMinutes(s);
  enemy.departAt = campaignMinutes(s);
  for (const g of s.nations.USA.groups.filter((g) => g.fleetId === enemy.id))
    g.atSea = false;
  f.route = [NODES.hawaii];
  f.port = "yokosuka";
  f.arriveAt = campaignMinutes(s);
  f.departAt = campaignMinutes(s);
  f.phase = "patrol";
  f.mission = "anchorage";
  f.objectiveNode = "hawaii";
  for (const g of n.groups.filter((g) => g.fleetId === f.id)) g.atSea = true;
  assert.ok(anchoredShips(s, c, "USA", "hawaii").length);
  const before = portSummary(s, c, "hawaii"),
    report = resolvePortActionToEnd(s, c, "JPN", f, "hawaii", "anchorage");
  assert.ok(report);
  assert.ok(report.resultB.damaged > 0);
  assert.ok(report.portDamage > 0);
  assert.match(battleDetails(report), /sailors lost/);
  assert.ok(portSummary(s, c, "hawaii").capacity < before.capacity);
  const health = s.ports.hawaii.health,
    gold = s.nations.USA.gold;
  repairPorts(s);
  assert.equal(s.ports.hawaii.health, health);
  assert.equal(s.nations.USA.gold, gold);
  setCampaignMinutes(s, campaignMinutes(s) + PORT_REPAIR.safeMinutes + 1);
  repairPorts(s);
  assert.ok(s.ports.hawaii.health > health);
  assert.ok(s.nations.USA.gold < gold);
  s.nations.USA.gold = 0;
  const stopped = s.ports.hawaii.health;
  repairPorts(s);
  assert.equal(s.ports.hawaii.health, stopped);
  assert.doesNotThrow(() => validateSave(s, b));
  s.relations["JPN-USA"].war = false;
  assert.equal(resolvePortActionToEnd(s, c, "JPN", f, "hawaii", "siege"), null);
});
test("admirals choose enemy ports for raid and siege and produce minute-stamped actions", () => {
  for (const mission of ["anchorage", "siege"]) {
    const s = start("JPN"),
      c = contentFor(b, s),
      f = s.nations.JPN.fleets.find((f) => f.role === "carrier");
    s.relations["JPN-USA"].war = true;
    s.relations["JPN-USA"].warSince = s.day;
    orderFleet(s, c, f.id, mission, null, "JPN", { aggressiveBattle: true });
    assert.ok(PORTS[f.objectiveNode]);
    assert.equal(PORTS[f.objectiveNode].nation, "USA");
    f.route = [NODES[f.objectiveNode]];
    f.targetNode = f.objectiveNode;
    f.phase = "patrol";
    f.arriveAt = campaignMinutes(s);
    f.departAt = campaignMinutes(s);
    f.nextPlanAt = campaignMinutes(s) + 1440;
...
test("all zoom levels preserve national convoy color, port icons and labels; manifests sort capitals first and newest within type", () => {
  ...
  for (let i = 1; i < ships.length; i++) {
    const a = c.classes[ships[i - 1].classId],
      b = c.classes[ships[i].classId];
    if (a.type === b.type) assert.ok(a.year >= b.year);
  }
  assert.ok(
    !s.nations.JPN.groups.some(
      (g) => /prototype/i.test(g.id) || /prototype/i.test(g.name),
    ),
  );
  const html = resourcesView(s, c);
  assert.ok(
    html.indexOf("Naval industry") < html.indexOf("Aircraft factories"),
  );
  assert.ok(html.indexOf("Aircraft factories") < html.indexOf("Naval schools"));
  assert.ok(
    html.indexOf("Naval schools") < html.indexOf("Naval aviation schools"),
  );
});
