...
export function shippingRisk(s,c,id,route,destination){
 const n=s.nations[id],now=campaignMinutes(s),danger=n.contacts.filter(x=>s.relations[pair(id,x.nation)]?.war&&now-x.seenAt<2880);
 let risk=s.ports[destination]?.blockade||0;
 for(const x of danger){const close=route.some((p,i)=>distanceNm(p,x.position)<120||i&&distanceNm(p,x.position)+distanceNm(route[i-1],x.position)<distanceNm(p,route[i-1])+80);if(!close)continue;
  const escort=n.fleets.filter(f=>f.mission==='guard'&&!['port','refuel','repair'].includes(f.phase)&&distanceNm(fleetPosition(s,f),x.position)<120).reduce((v,f)=>v+n.groups.filter(g=>g.fleetId===f.id&&g.atSea&&g.status==='active').reduce((v,g)=>v+g.count,0),0);
  risk=Math.max(risk,Math.min(.95,(x.estimate||1)/(5+escort*2)));
 }return risk;
}
...
function putDestination(s,c,id,t){
 ...
 for(const w of t.airWing){
  ...
  if(t.replace&&(base||g&&!g.atSea)){const storage=base||n.airBases[g.dockPort||n.fleets.find(f=>f.id===g.fleetId)?.port];const old=wings.find(x=>x.role===w.role&&x.model!==w.model);if(old){const count=Math.min(w.count,old.count),crewed=Math.min(count,old.crewed);addWing(storage.reserve,{...old,count,crewed});old.count-=count;old.crewed-=crewed;}}
  const take=Math.min(w.count,Math.max(0,capacity-total(wings))),crewed=Math.min(take,w.crewed);addWing(wings,{...w,count:take,crewed});w.count-=take;w.crewed-=crewed;
  if(w.count){const port=base?t.destination:g?.dockPort||n.fleets.find(f=>f.id===g?.fleetId)?.port,store=n.airBases[port];if(store&&aviationOwner(s,port)===id&&(base||!g.atSea&&distanceNm(shipAirLocation(s,n,g),NODES[port])<25))addWing(store.reserve,w);else return false;w.count=0;w.crewed=0;}
 }
 if(base)base.supplies=Math.min(aviationStockCapacity(s,t.destination),base.supplies+t.supplies);
...
export function dispatchAviation(s,c,id,{source=airWarehouse(s,id),destination,model=null,role='strike',count=0,supplies=0,replace=false}={}){
 ...
 if(mode==='merchant'){
  if(!destinationPort){destinationPort=Object.keys(n.airBases).filter(p=>aviationOwner(s,p)===id&&ferryPath(s,c,id,p,destination,a)).sort((x,y)=>distanceNm(NODES[source],NODES[x])-distanceNm(NODES[source],NODES[y]))[0];if(!destinationPort)return null;}
  route=seaRoute(source,destinationPort).map(k=>NODES[k]);if(shippingRisk(s,c,id,route,destinationPort)>.6){n.airBases[destinationPort].heldReason='Known enemy forces or blockade make merchant reinforcement unsafe.';return null;}
  const free=merchantCount(s,id)-n.convoys.reduce((v,x)=>v+x.count,0);shipCount=Math.min(3,free);if(shipCount<1||n.convoys.length>=90)return null;
 }
 const wing=count?sourceWing(s,c,id,model,role,count,source):null;if(wing&&(!wing.count||wing.crewed<wing.count)){if(wing.count)addWing(n.airBases[source].reserve,wing);return null;}
 const t={id:'air-transfer-'+s.nextId++,source,destination,mode,airWing:wing?[wing]:[],supplies:suppliesExpense(n,supplies),replace,departAt:now,arriveAt:now,route:route||[NODES[source],endpoint(s,n,destination)],path:flight||[],leg:0};
 if(!total(t.airWing)&&!t.supplies)return null;
 if(mode==='ferry'&&flight.length>1){if(!beginLeg(s,c,id,t)){for(const w of t.airWing)addWing(n.airBases[source].reserve,w);return null;}}
 if(mode==='rail')t.arriveAt=now+1440*(1+distanceNm(NODES[source],NODES[destination])*1.852/500);
 if(mode==='merchant'){
  t.finalDestination=destination;t.destination=destinationPort;t.arriveAt=now+routeLength(route)/10*60+720;t.convoyId='convoy-air-'+s.nextId++;t.lastCount=shipCount;
  n.convoys.push({id:t.convoyId,nation:id,name:'Aviation transport → '+PORTS[destinationPort].name,count:shipCount,port:source,node:destinationPort,targetNode:destinationPort,route,departAt:now,arriveAt:t.arriveAt,speed:10,lastBattle:-1e9,cargo:t.supplies+total(t.airWing)*10,aviationTransfer:t.id});
 }
 n.airTransfers.push(t);if(n.airBases[destinationPort])delete n.airBases[destinationPort].heldReason;return t;
}
function failTransfer(s,c,id,t){const n=s.nations[id];
 ...
 const last=t.mode==='ferry'?t.path[Math.min(t.leg,t.path.length-1)]:t.source,base=n.airBases[last];
 if(base&&aviationOwner(s,last)===id&&t.mode==='rail'){for(const w of t.airWing)addWing(base.reserve,w);t.airWing=[];aviationLog(s,n,'Ferry flight held or diverted at '+PORTS[last].name+'. Destination moved beyond range or became unavailable.');}
 else loseAircraft(s,c,id,t,1,{rescue:.2,airframeRescue:0});
 t.supplies=0;t.done=true;
}
export function minuteAviation(s,c){
 ...
 for(const [id,n]of Object.entries(s.nations)){
  for(const t of n.airTransfers||[]){
   if(t.mode==='merchant'){
    const convoy=n.convoys.find(v=>v.id===t.convoyId),remaining=convoy?.count||0;
    if(remaining<t.lastCount){const fraction=1-remaining/t.lastCount;const cover=convoyCoverage(s,c,id).convoys.find(v=>v.id===t.convoyId),rescue=Math.min(.75,.15+(cover?.escorts.length||0)*.12);const loss=loseAircraft(s,c,id,t,fraction,{rescue,airframeRescue:0});const notice=s.alerts.find(x=>x.kind==='convoy'&&x.b===id&&x.minute===now);if(notice)notice.body+=' Aviation cargo: '+loss.planes+' aircraft lost; '+loss.aviators+' aviators lost and '+loss.aviatorsRescued+' rescued.';t.supplies*=1-fraction;t.lastCount=remaining;aviationLog(s,n,'Aviation transport intercepted: aircraft, aircrews and supplies lost in proportion to shipping sunk.');}
    if(!remaining){t.done=true;continue;}
    if(aviationOwner(s,t.destination)!==id){const home=airWarehouse(s,id);if(home&&t.destination!==home){const position=fleetPosition(s,convoy),nearest=Object.keys(NODES).sort((a,b)=>distanceNm(position,NODES[a])-distanceNm(position,NODES[b]))[0];t.destination=home;delete t.finalDestination;convoy.targetNode=home;convoy.node=home;convoy.route=[position,...seaRoute(nearest,home).map(k=>NODES[k])];convoy.departAt=now;convoy.arriveAt=now+routeLength(convoy.route)/10*60;t.arriveAt=convoy.arriveAt;}else{failTransfer(s,c,id,t);continue;}}
   }
   ...
   if(t.finalDestination&&t.finalDestination!==t.destination){const base=n.airBases[t.destination];for(const w of t.airWing)addWing(base.reserve,w);t.airWing=[];t.done=true;}
   else if(putDestination(s,c,id,t)){t.done=true;aviationLog(s,n,(t.mode==='merchant'?'Merchant reinforcement':t.mode==='rail'?'Domestic reinforcement':'Ferry flight')+' arrived at '+(PORTS[t.destination]?.name||n.groups.find(g=>g.id===t.destination)?.name)+'.');}
   else failTransfer(s,c,id,t);
  }
  const done=new Set((n.airTransfers||[]).filter(t=>t.done).map(t=>t.id));if(done.size){n.convoys=n.convoys.filter(v=>!done.has(v.aviationTransfer));n.airTransfers=n.airTransfers.filter(t=>!t.done);staffAircraft(s,c,id);}
 }
}
export function dailyAviation(s,c){
 ...
 for(const [id,n]of Object.entries(s.nations)){
  ...
  for(const [port,base]of Object.entries(n.airBases)){
   ...
   if(!landSupplied(id,port)&&base.supplies<aviationStockCapacity(s,port)*.65&&!n.airTransfers.some(t=>t.destination===port&&t.supplies>0)){
