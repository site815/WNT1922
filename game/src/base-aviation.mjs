...
import { aircraftModels, operationalAircraftModels, aircraftSeats, planeRole } from './naval-resources.mjs';
...
export function allocatedWings(n){return [...n.groups.flatMap(g=>g.airWing||[]),...Object.values(n.airBases||{}).flatMap(b=>[...b.airWing,...(b.governmentWing||[]),...b.reserve]),...[...(n.airTransfers||[]),...(n.airSorties||[])].flatMap(t=>t.airWing)];}
export function freeAircraft(n){const free={...n.aircraft,...n.governmentAircraft};for(const w of allocatedWings(n))free[w.model]=Math.max(0,(free[w.model]||0)-w.count);return free;}
...
export function baseAirPower(s,c,port,distanceKm=0){
 ...
 const rows=[...base.airWing,...(base.governmentWing||[])],models=new Map(operationalAircraftModels(c,id).map(a=>[a.id,a])),aircraft=rows.reduce((v,w)=>v+w.count,0),crewed=rows.reduce((v,w)=>v+(w.crewed||0),0);
 const readiness=health*Math.min(1,base.supplies/Math.max(1,crewed));let strike=0,fighters=0,scout=0,radius=0,sorties=0;
 const wings=rows.map(w=>{const a=models.get(w.model),range=modelCombatKm(a),quality=1+Math.max(0,(a?.type_year||1922)-1930)*.035,effective=(w.crewed||0)*readiness;
  ...
  return {...w,government:a?.catalogKind==='government',name:a?.name||w.model,combatKm:range,ferryKm:modelFerryKm(a)};
 });
...
export function flyBaseSorties(s,c,port,distanceKm){
 ...
 if(!base||campaignMinutes(s)<power.readyAt||!airConditions(s,NODES[port]).launch)return {...power,strike:0,fighters:0,scout:0,sorties:0};
 base.supplies=Math.max(0,base.supplies-power.sorties);base.lastSortie=campaignMinutes(s);const factor=airConditions(s,NODES[port]).launch;return {...power,strike:power.strike*factor,fighters:power.fighters*factor};
}
