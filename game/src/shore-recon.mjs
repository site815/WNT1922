...
export function coastalRecon(s,c){
 ...
 for(const [id,n]of Object.entries(s.nations))for(const port of Object.keys(n.airBases||{})){
  if(aviationOwner(s,port)!==id)continue;const air=baseAirPower(s,c,port),conditions=airConditions(s,NODES[port]);if(air.scout<=0||air.readiness<=0||conditions.light!=='Daylight'||!conditions.launch)continue;
  const sector=searchSector(s,{salt:Object.keys(PORTS).indexOf(port)},NODES[port]);
  for(const target of forces){if(target.id===id||Math.abs(target.position[1]-NODES[port][1])*111.2>air.radius)continue;
   ...
   const chance=Math.min(.8,.08+search*.018)*Math.max(.1,1-distance/Math.max(1,air.radius)*.8)*conditions.search*sectorFactor(sector,NODES[port],target.position);
  ...
  for(const enemyPort of Object.keys(PORTS)){
   const enemy=aviationOwner(s,enemyPort);if(!s.relations[pair(id,enemy)]?.war||distanceNm(NODES[port],NODES[enemyPort])*1.852>air.radius)continue;
   if(operationRandom(s)<.35*conditions.search*sectorFactor(sector,NODES[port],NODES[enemyPort]))observeAnchorage(s,c,id,enemyPort,'Shore patrol');
  }
 }
 ...
 for(const {id,f,position}of forces){
  if(['port','refuel','repair'].includes(f.phase))continue;const st=fleetStats(s,c,id,f),conditions=airConditions(s,position);if(st.scouts<=0||!conditions.launch)continue;
  const sector=searchSector(s,f,position,NODES[f.objectiveNode]);
  for(const port of Object.keys(PORTS))if(s.relations[pair(id,aviationOwner(s,port))]?.war&&distanceNm(position,NODES[port])<=st.airRadius&&operationRandom(s)<.45*conditions.search*sectorFactor(sector,position,NODES[port]))observeAnchorage(s,c,id,port,'Fleet aerial reconnaissance');
 }
}
export function observeAnchorage(s,c,id,port,source){
 const owner=aviationOwner(s,port),ships=anchoredShips(s,c,owner,port),composition={};let hulls=0;
 for(const g of ships){hulls+=g.count;const type=c.classes[g.classId].type;composition[type]=(composition[type]||0)+g.count;}
 s.nations[id].anchorageReports??={};s.nations[id].anchorageReports[port]={seenAt:campaignMinutes(s),owner,hulls,composition,source};
}
