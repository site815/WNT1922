...
export function contactDescription(s,c){
  const t=capitalClock(s,s.player,c.seenAt),[lon,lat]=c.position;
...
export function contactAlerts(s){
  return visibleContacts(s).filter(c=>s.relations[[s.player,c.nation].sort().join('-')]?.war&&c.hours<=INTELLIGENCE.uncertainHours&&!Number.isFinite(c.dismissedAt)).map(c=>({
    id:'contact-'+c.id,contactId:c.id,minute:c.seenAt,kind:'contact',
