...
export function fleetReadinessHover(s,c,f){
 ...
 return '<span class="eyebrow">FLEET READINESS</span><h3>'+esc(f.name)+'</h3>'+list([['Training / morale',num(n.training)+'% / '+num(n.morale)+'%'],['Supply',num(supply.factor*100)+'%'],['Hull condition',num(stats.health*100)+'%'],['Remaining endurance',num(f.fuelNm*1.852)+' km (fleet fuel limit)'],['Speed / maximum',num(f.speed,1)+' / '+num(stats.maxSpeed,1)+' kn'],['Status',esc(fleetStatus(s,f))],['Air operations',esc(airOperationsText(s,c,s.player,f))],['Engagement policy',f.role==='support'?'Automatic support':f.aggressiveBattle?'Seek aggressive battle':'Preserve the force at poor odds']])+fleetHoverManifest(s,c,f)+'<small>Click the fleet to center it and open its orders in the command panel.</small>';
}
...
export function portPopup(s,c,id,{parts=false}={}){
 ...
 if(p.owner!==s.player){const report=s.nations[s.player].anchorageReports?.[id],hours=report?Math.max(0,Math.floor((campaignMinutes(s)-report.seenAt)/60)):null,body=list([['Charted supply capacity',num(portSpec(s,id).capacity)+' t'],['Trade capacity (charted)',num(p.trade)],['Coastal battery',esc(p.battery)],['Charted gun radius',num(p.gunRange*1.852)+' km'],['Anchorage reconnaissance',report?num(hours)+' hours old · '+esc(report.source):'No aerial observation'],['Last observed ships',report?compositionText(report.composition,'Harbor observed empty'):'Unknown']]),notes='<p>Foreign aircraft, stores and ships are not a live intelligence feed. Aerial reconnaissance produces dated anchorage reports; admirals use observations less than 48 hours old to plan raids.</p>';return parts?{summary,body,notes}:summary+body+notes;}
 const body=list([
