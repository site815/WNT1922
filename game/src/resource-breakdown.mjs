...
import { growthOutlook } from './economic-growth.mjs';
...
export function resourceHover(s,c,key){
 ...
 if(key==='GOLD'||key==='INDUSTRY'){
  ...
  const account=n.monthAccount,k=gold?'gold':'industry',growth=growthOutlook(s,c);
  rows.push(flow('Actual change this month',n[k]-(account?.opening[k]??n[k])),['Last completed month',account?.last?signed(account.last[k]):'No complete month'],['Economic base index',num((n.economicIndex??1)*100,2)+' / opening 100'],['Next monthly base growth',signed(growth.monthly*100)+'%']);
  for(const r of growth.rows)rows.push(['Growth: '+r.label,signed(r.annual*100)+'% / year']);
  ...
  rows=[['Domestic share',pct(e.domestic)],['Trade share',pct(e.trade)],['Merchant capacity factor','× '+pct(e.coverage)],['Convoy flow factor','× '+pct(n.commerce/100)],['Usable port factor','× '+pct(e.ports.coverage)],['Realized trade',pct(e.tradeFlow)],['Total economy factor',pct(e.economyFactor)],['Monthly economic growth',signed(growthOutlook(s,c).monthly*100)+'%']];note='Domestic share + trade share × merchant coverage × convoy flow × port access. Trade also affects monthly growth of the real economic base. Trade flow cannot exceed 100%; excess shipping provides loss tolerance.';
 }else if(key==='SHIPPING'){
  rows=[['Available merchant volume',num(e.current)+' GRT'],['Opening shipping requirement',num(e.baseline)+' GRT'],flow('Industry expansion demand',e.required-e.baseline),['Total requirement',num(e.required)+' GRT'],['Capacity coverage',pct(e.coverage)],['Civilian deliveries this month',num(n.civilianShipping?.last?.hulls)+' hulls'],['Civilian deliveries in campaign',num(n.civilianShipping?.delivered)+' hulls'],['Civilian growth outlook',pct(growthOutlook(s,c).merchantAnnual)+' / year'],['Merchant hulls lost',num(n.merchantLost)]];note='Each naval-industry upgrade adds 15% of opening merchant demand. Civilian yards deliver whole merchant hulls monthly, independently of naval orders. Trade and shipping demand support growth; war slows it. GRT is registered volume, separate from naval displacement.';
  ...
  const a=v?.air||aircraftSummary(s,c);rows=[['Total owned',num(a.total)],['Embarked on ships',num(a.assigned)],['Stationed at bases',num(a.stationed)],['Ferry / merchant transfer',num(a.transit-(a.airborne||0))],['Airborne combat sorties',num(a.airborne)],['Unassigned reserve',num(a.reserve)],['Without full aircrews',num(a.uncrewed)],['Factory output last day',num(n.aircraftOutput)],['Salvage awaiting repair',num(awaitingRecovery(n,'aircraft'))],['Permanently lost',num(n.casualties.aircraft.lost)]];note='Naval inventory conserves airframes across ships, bases, transfers, combat sorties and reserve. Other-service maritime aircraft and crews have a separate government establishment. New models replace older qualified models when a delivery route is available.';
 }
