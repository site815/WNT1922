...
test('every campaign and navy can order both support types without adding opening warships',()=>{
 for(const campaign of ['campaign_1922','in_good_faith_1936'])for(const id of ['GBR','USA','JPN','FRA','ITA','DEU','SOV']){const s=start(id,campaign),c=contentFor(bundle,s),before=sim.fleetSummary(s,c).total;for(const type of ['AD','AO']){const cl=c.nations[id].designs.map(id=>c.classes[id]).find(cl=>cl.type===type&&!productionBlock(s,c,cl.id)&&cl.id.includes(sim.yearOf(s)));assert.ok(cl,id+' '+campaign+' '+type);const group=sim.orderShip(s,c,cl.id);assert.equal(s.nations[id].groups.find(g=>g.id===group).service,'support');}assert.equal(sim.fleetSummary(s,c).total,before);}
});
...
function deliverSupport(s,c,type){const n=s.nations[s.player];n.crew=1e7;const id=sim.orderShip(s,c,'us_'+type+'_1936'),g=n.groups.find(g=>g.id===id);g.status='active';g.dockPort='hawaii';organizeSupport(s,c);const f=n.fleets.find(f=>f.id===g.fleetId);assert.equal(f.role,'support');return {g,f};}
