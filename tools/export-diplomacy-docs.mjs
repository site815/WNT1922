...
import {RELATIONS_1922,RELATIONS_1936,relationKey} from '../game/src/diplomacy-history.mjs';
import {NATION_ORDER} from '../game/src/catalog.mjs';
const signed=x=>(x>=0?'+':'')+x;
let table='| Pair | 1922 relations | 1922 historical change / month | 1936 relations | 1936 historical change / month |\n|---|---:|---:|---:|---:|\n';
for(let i=0;i<NATION_ORDER.length;i++)for(let j=i+1;j<NATION_ORDER.length;j++){
 const a=NATION_ORDER[i],b=NATION_ORDER[j],key=relationKey(a,b),x=RELATIONS_1922[key],y=RELATIONS_1936[key];
 table+='| '+a+' / '+b+' | '+[x[0],x[1],y[0],y[1]].map(signed).join(' | ')+' |\n';
}
const text=`# Diplomacy, war warnings and alliances
...
`;
const file='docs/diplomacy.md';if(process.argv.includes('--check'))assert.equal(fs.readFileSync(file,'utf8'),text,'Diplomacy document is stale');else fs.writeFileSync(file,text);
console.log('Diplomacy rules and 42 opening pair scores '+(process.argv.includes('--check')?'verified.':'documented.'));
