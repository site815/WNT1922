...
export function topBars(s, c, m = {}, alerts = '') {
  ...
  const stats = [
    ...
    ["GDP", compact(income.products.gdp), "GDP naval budget: annual domestic-funded ministry budget in kg fine-gold equivalent; not total national GDP"],
    ["GTP", compact(e.gtp), "GTP naval budget: annual trade-funded ministry budget in kg fine-gold equivalent; changes through monthly logistics growth only"],
  ...
  const speed = s.paused
    ...
    : m.warming
      ? "Measuring…"
      : num(m.actual) + "× actual";
  return `<header class="command-bar time-bar"><strong class="campaign-clock">${clock.date} <b>${clock.time}</b> <small>${clock.capital} · ${clock.zone}</small></strong>${alerts}<div class="clock-controls"><span class="actual-speed ${s.paused ? "is-paused " : ""}${s.paused || m.warming ? "" : m.ratio < 0.65 ? "slow-red" : m.ratio < 0.9 ? "slow-yellow" : ""}" title="Measured simulation speed. Slows safely under load; every fifteen-minute tick is simulated.">${speed}</span><button class="step-control" data-action="step-minute" ${s.paused ? "" : 'disabled data-disabled-reason="Pause to advance time in steps."'} title="Advance fifteen simulated minutes">+15m</button><button class="step-control" data-action="step-six-hours" ${s.paused ? "" : 'disabled data-disabled-reason="Pause to advance time in steps."'} title="Advance six simulated hours">+6h</button><button data-action="pause" class="pause-control" title="Pause or resume. Autopause controls interruptions from decisions, war and leaving the window.">${s.paused ? "▶ Resume" : "Ⅱ Pause"}</button><select id="speed" aria-label="Simulation speed">${SPEEDS.map(([v, t]) => `<option value="${v}" ${s.speed === v ? "selected" : ""}>${t}</option>`).join("")}</select></div><div class="header-audio"><label class="autopause-control" title="Checked: decisions and war pause for your attention. Unchecked: simulation mode continues through events and in the background; pending choices use their deadline defaults unless you respond."><input id="auto-pause" type="checkbox" ${s.autoPause ? "checked" : ""}>Autopause</label><label title="Sound effects"><input id="sound-enabled" type="checkbox" ${s.audioEnabled ? "checked" : ""}>SFX</label><input id="sound-volume" aria-label="Sound effects volume" type="range" min="0" max="100" value="${s.audioVolume * 100}"><label title="Music"><input id="music-enabled" type="checkbox" ${s.musicEnabled ? "checked" : ""}>Music</label><input id="music-volume" aria-label="Music volume" type="range" min="0" max="100" value="${s.musicVolume * 100}"><button data-action="music-next" title="Next track · now ${esc(musicStatus().track.title)}" aria-label="Next music track">▶|</button></div></header><div class="resources resource-bar" style="--resource-count:${stats.length + (wars.length ? 1 : 0)}" aria-label="National resources">${stats.map(([label, value, hint, negative]) => `<div class="${negative ? "negative" : ""}" data-resource="${esc(label)}" tabindex="0" aria-label="${esc(label + ": " + hint)}"><label>${["GDP","GTP"].includes(label)?label+" BUDGET":label}</label><strong>${value}</strong><small class="resource-change ${changes[label]?.[1]<0?"negative":changes[label]?.[1]>0?"positive":""}">${changes[label]?.[0]||""}</small></div>`).join("")}${wars.length ? `<div class="resource-war" title="${esc(wars.map((w) => w.name + ": " + w.result + " · war balance " + num(w.score) + " = naval " + num(w.naval) + " + land " + num(w.land) + ". Naval balance values sunk tonnage + 35% damaged tonnage + 15% merchant GRT. Leading/trailing compares losses and land progress; it does not predict victory.").join(" · "))}"><label>AT WAR · BALANCE</label><strong>${wars.map((w) => w.opponent + " " + (w.result === "Leading" ? "↑" : w.result === "Trailing" ? "↓" : "↔")).join(" · ")}</strong></div>` : ""}</div>`;
}
