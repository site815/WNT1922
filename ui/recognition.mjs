...
function artworkImage(entry, expanded = false, actualSize = false) {
  ...
  return crop ? `<div class="recognition-crop" style="aspect-ratio:${crop.width}/${crop.height};${actualSize ? `width:${crop.width}px;min-width:100%` : "width:100%"}">${image}</div>` : image;
}
function caption(entry, note = "") {
  const source = entry.source || {};
  return `<figcaption><strong>${esc(entry.title)}</strong><span>${esc((entry.views || []).join(" · "))} · ${entry.origin === "historical" ? "Historical reference" : "Original interpretation"}</span><span>${esc(entry.configuration)}</span>${note ? `<span>${esc(note)}</span>` : ""}<small>${esc(source.author)} · ${sourceLink(/^https?:\/\//.test(source.page || "") ? source.page : source.url, source.title)}${source.page && !/^https?:\/\//.test(source.page) ? " · " + esc(source.page) : ""}${source.credit ? " · " + esc(source.credit) : ""} · ${sourceLink(source.licenseUrl, source.license)}${source.modifications ? " · " + esc(source.modifications) : ""}</small></figcaption>`;
}
export function recognitionCard(kind, id, { compact = false, campaign = "" } = {}) {
  ...
  const match = current.platforms.get(campaign + ":" + kind + ":" + id) || current.platforms.get(kind + ":" + id);
...
export function recognitionExpanded(id, { actualSize = false } = {}) {
  ...
  return entry ? `<button data-action="recognition-zoom" data-id="${esc(entry.id)}">${actualSize ? "Fit drawing" : "Actual size"}</button><figure class="recognition-card expanded${entry.display?.monochrome ? " monochrome" : ""}" data-recognition="${esc(entry.id)}"><div class="recognition-sheet${actualSize ? " actual-size" : ""}" data-scroll-key="recognition-${esc(entry.id)}">${artworkImage(entry, true, actualSize)}</div>${caption(entry)}</figure><p class="panel-note">Choose actual size to inspect fine details with the scrollbars. Artwork records the named configuration; campaign performance uses the catalog specifications.</p>` : '<p>Recognition drawing unavailable.</p>';
}
export function recognitionCredits() {
  if (!current) return `<p>${esc(failure || "Loading recognition credits…")}</p>`;
  return `<p>${current.entries.size} recognition drawings cover ${current.platforms.size} catalog platforms. Historical plates and original interpretations are identified individually.</p><div class="recognition-credits">${[...current.entries.values()].map(entry => `<figure>${caption(entry)}</figure>`).join("")}</div>`;
}
