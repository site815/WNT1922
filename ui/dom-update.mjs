...
function sync(old, next) {
  ...
  for (const a of [...old.attributes])
    if (
      !next.hasAttribute(a.name) &&
      !(a.name === "open" && old.tagName === "DETAILS")
    )
...
function reconcile(old, next) {
  const previous = [...old.childNodes],
    used = new Set();
  let cursor = old.firstChild;
  for (const child of [...next.childNodes]) {
    let match =
      cursor && identity(cursor) === identity(child) && !used.has(cursor)
        ? cursor
        : previous.find((n) => !used.has(n) && identity(n) === identity(child));
    if (match) {
      used.add(match);
